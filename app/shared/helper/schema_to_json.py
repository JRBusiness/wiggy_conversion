...
def generate_data_for_model(model: BaseModel, fake: Faker) -> dict:
    ...
    if not issubclass(type(model), Schema.__class__):
        print(
            TypeError("Invalid argument: First argument must be a Pydantic model class")
        )
        return
        ...
        if isinstance(field_type, BaseModel.__class__):
            if issubclass(field_type, BaseModel.__class__):
                ...
                if issubclass(inner_type, BaseModel.__class__):
                    data[key] = [
                        generate_data_for_model(inner_type, fake) for _ in range(3)
                    ]
                else:
                ...
                data[key] = tuple(
                    fake.parse(inner_type.__name__) for inner_type in inner_types
                )
            else:
...
def generate_sample_data(module_name: str) -> dict:
    ...
    return {model.__name__: generate_data_for_model(model, fake) for model in models}
